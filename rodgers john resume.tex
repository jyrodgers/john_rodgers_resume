...
\begin{itemize}
    ...
    \begin{itemize}
	    ...
	    \item Accelerated development by implementing ccache in \textbf{Docker} build environment, cutting down data transfer with \textbf{volume mounts} to the host.
	...
	\item Ensured compliance with rigorous security standards in cross-virtual machine communications by designing and owning a \textbf{C++} library that managed \textbf{OpenSSL}-secured \textbf{DTLS} connections.
        \begin{itemize}
	        \item Prioritized developer convenience with argument-based config, allowing simple setup of encryption, timeout, SSL roles, and addresses abstracting away the complexities of underlying implementation.
	        \item Enhanced connection stability by integrating \textbf{multi-threading} for efficient and reliable heartbeat monitoring, facilitating automatic re-establishment of SSL connections for continuous operation.
